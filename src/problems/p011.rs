...
pub fn main() {
    ...
    let quads = {
        ...
        .chain(down_bases.map(down))
        .chain(other_diag_bases.map(other_diag))
    };
    
    let Some(maxi) = quads.map(
        |xs| xs
            .map(|x| GRID[x.y][x.x])
            .product::<u32>())
        .max() else {
...
fn down(p: Point) -> std::array::IntoIter<Point, 4> {
    [
        p,
        Point {
            x: p.x,
...
fn right(p: Point) -> std::array::IntoIter<Point, 4> {
    [
        p,
        Point {
            x: p.x + 1,
...
fn diag(p: Point) -> std::array::IntoIter<Point, 4> {
    [
        p,
        Point {
            x: p.x + 1,
...
fn other_diag(p: Point) -> std::array::IntoIter<Point, 4> {
    [
        p,
        Point {
            x: p.x - 1,
