...
For now, the update system for when I add a new problem solution is by adding the appropriate lines to the various files. This process is automated by [`add_problem.rs`](add_problem.rs) which I compile directly with rustc on whichever platform I'm on. Again, I let Claude.ai do most of that work for me and just had to correct a few small things.

Following a change in to track potentially incomplete solutions, A similar, but simpler automatation tool [`status_update.rs`](status_update.rs) also exists and was again made by Claude.ai.